...
pub(crate) fn plugin(app: &mut App) {
    ...
    app.add_systems(OnEnter(GameState::Tutorial), spawn_hand_sprites)
        .add_systems(OnEnter(GameState::Prepare), spawn_hand_sprites)
        .add_systems(OnExit(GameState::Prepare), hide_hand_sprites)
        .add_systems(OnEnter(GameState::Start), hide_right_hand)
        .add_systems(OnEnter(GameState::Tutorial), show_right_hand)
        .add_systems(OnEnter(GameState::Prepare), show_right_hand);
    #[cfg(feature = "backend")]
    app.add_systems(
        Update,
        sync_hands_to_palm_detections.run_if(
            in_state(GameState::Prepare)
                ...
                .or_else(in_state(GameState::Start))
...
fn hide_right_hand(mut right: Query<&mut Visibility, With<RightHand>>) {
    if let Ok(mut rv) = right.single_mut() {
        rv.set_if_neq(Visibility::Hidden);
    }
}

fn show_right_hand(mut right: Query<&mut Visibility, With<RightHand>>) {
    if let Ok(mut rv) = right.single_mut() {
...
fn sync_hands_to_palm_detections(
    ...
    let in_start = game_state.is_some_and(|s| *s == GameState::Start);
